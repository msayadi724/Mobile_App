...
const fs = require('fs');
const cert1 = fs.readFileSync('./tls/token-public-key.pem');
...
exports.login = (req, res) => {
    try {
        ...
        res.status(201).send({accessToken: token, refreshToken: refresh_token , jwt : req.body});
    } catch (err) {
...
exports.refresh_token = (req, res) => {
    try {
        ...
        let token = jwt.sign(req.body,cert, { algorithm: 'RS512'});
        res.status(201).send({access_token: token , changed : true , jwt : req.body});
    } catch (err) {
        res.status(500).send({errors: err});
    }
};
